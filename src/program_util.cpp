...
void ProgramUtil::migrateOutputCell( Program& p, int64_t old_out, int64_t new_out )
{
  ...
  {
    if ( p.ops[found_mov_to_old].source == Operand( Operand::Type::DIRECT, new_out ) )
    {
      p.ops.erase( p.ops.begin() + found_mov_to_old, p.ops.begin() + found_mov_to_old + 1 );
      found_mov_to_old--;
    }
    else
    {
  ...
  {
    p.push_back( Operation::Type::MOV, Operand::Type::DIRECT, new_out, Operand::Type::DIRECT, old_out );
  }
}
