...
set output '../operation_type_counts.png'
set datafile separator ","
set boxwidth 0.5
set style fill solid
unset key
set title "LODA Operation Type Distribution"
plot "${statshome}/operation_type_counts.csv" using 2:xtic(1) with boxes linecolor rgb "#0000FF"
EOF
...
cat << EOF > lengths.gp
set terminal pngcairo font "arial,9" size 600,300
set output '../program_lengths.png'
set datafile separator ","
set boxwidth 0.75
set style fill solid
set xrange [0:30]
...
plot "${statshome}/program_lengths.csv" using 2:xtic(1) with boxes linecolor rgb "#0000FF"
EOF
...
} < ${statshome}/cal_graph.csv
echo "}" >> cal_graph.dot
neato -Tpng cal_graph.dot -o tmp.png
convert tmp.png ../cal_graph.png
